...
# cursor.execute("CREATE TABLE listingsDec(id,name,description,transit,picture_url,host_id,host_url,street,
# neighbourhood,city,state,zipcode,market,smart_location,property_type,room_type,accommodates,bathrooms,bedrooms,
# beds,price,weekly_price,monthly_price,security_deposit,cleaning_fee,guests_included,extra_people,
...
def get_the_data():
    ...
    for row in df_listings.itertuples():
        ...
        # values = (row.id,row.name,row.description,row.transit,row.picture_url,row.host_id,row.host_url,row.street,
