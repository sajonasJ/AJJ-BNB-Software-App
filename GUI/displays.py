...
def displaysuburblistings(results, columnNames, suburb):
    ...
    newWindow.maxsize(max_width, max_height)
    ...
    newWindow.geometry(f"{initial_width}x{initial_height}")

    # Treeview Configuration
    tree = ttk.Treeview(newWindow, column=columnNames, show='headings')
    max_widths = [max(len(str(row[i])) for row in results) for i in range(len(columnNames))]
    min_width = 50
    max_col_width = 200
